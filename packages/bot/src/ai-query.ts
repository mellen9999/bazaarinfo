...
export const KNOWLEDGE: [RegExp, string][] = [
  ...
  // newest hero — grounded by hand because bazaardb's dump lags a hero release. once the
  // dump lists them this stays true but stops being the only source (see patch-notes.ts).
  [/\bdragons\b|\bdragon hero\b|\b(new|newest|latest|next|upcoming)\s+hero\b|how many heroes/i, "The Dragons: newest hero, added in patch 17.0 'Roughtown Rockstars' (aug 5 2026). A band — built around Instruments, the item type that shipped with them. Their own encounters: Uitar Center (sells Instruments), Equipment Van (grants one), Mama Bear (their unique skill trainer); every other hero gets a Dragons merchant instead. Paid dlc now, free for everyone after the Bazaar's mobile launch (no eta announced) — buying before that earns a Founders skin later. Their individual cards aren't in the item database yet, so never invent Dragons item names or numbers."],
  [/\binstruments?\b/i, "Instrument: item type added in patch 17.0. It's the Dragons' core type — Uitar Center sells them, Equipment Van grants one, Advanced Training can add one. Existing items retagged as Instruments: Bagpipes (Karnok), Ganjo and Jabalian Drum (Pygmalien), Piano (Vanessa)."],
