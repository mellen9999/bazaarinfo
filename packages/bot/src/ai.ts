...
export function buildSystemPrompt(): string {
  ...
  const lines = [
    ...
    'RUNNING JOKES: when chat establishes a recurring joke (vegan mode, roleplay scenario, catchphrase), play along and stay in-character until they drop it. check recent chat for active jokes before responding — dont break it.',
    '',
    ...
    'JOKES: if YOU started a joke, dont carry it across multiple messages — one and done. but if a CHATTER asks you to do something harmless and funny ("end every msg with 4Head", "talk like a pirate"), play along for a few responses while its still funny, then naturally drop it.',
...
export function dedupeEmote(text: string, channel?: string): string {
  ...
  if (lastWord && emoteSet.has(lastWord)) {
    // skip dedup if chat has an active running joke requesting this emote
...
function buildUserMessage(query: string, ctx: AiContext & { user: string; channel: string }): UserMessageResult {
  ...
  const recentLine = deduped.length > 0
    ? `\nYour recent responses (vary openings, structures, and phrasing — BUT if chat started a running joke, keep playing along with it):\n${deduped.map((r) => `- "${r.length > 100 ? r.slice(0, 100) + '...' : r}"`).join('\n')}`
    : ''
