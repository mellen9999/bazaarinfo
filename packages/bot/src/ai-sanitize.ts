...
import { log } from './log'
import { normalizeText, peelLeadingTriggers } from './text-safety'
...
// denying the stream-schedule capability ("kripp's schedule isn't something i track",
...
export const SCOPE_DODGE =/\b(?:wrong (?:lobby|game|channel|chat)|not my (?:lane|department|wheelhouse|area|thing)|i(?:'m| am)?\s+(?:strictly|only|just)\s+(?:a\s+|the\s+)?bazaar|only\s+(?:do|know|cover|answer|handle)\s+bazaar|(?:this|i)\s+(?:is|am)\s+(?:a\s+)?bazaar\s+(?:bot|chat|channel|stream),?\s+not|ask me\s+(?:something\s+)?about\s+bazaar\s+(?:instead|then)|stick to bazaar|bazaar (?:questions|stuff) only)\b/i
...
export function sanitize(text: string, asker?: string, privileged?: boolean, knownUsers?: Set<string>, truncated?: boolean, isRealUser?: (name: string) => boolean, hasStats?: boolean): { text: string; mentions: string[] } {
  ...
  // reject degenerate command-echo fragments. the bot is now allowed to post real commands
  // (so chat can play), but a reply that is ONLY a command trigger + a 1-2 char stub — e.g.
  // the model echoing its own invocation "!b" and nothing else, which would post as a bare
  // "b @user" — is not an answer. peel EVERY leading trigger (not just the dangerous ones the
