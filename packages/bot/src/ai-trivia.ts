...
- NAME THE SUBJECT — this is the #1 chat complaint. If the question opens with or leans on an unnamed specific thing ("this curved blade", "this game", "this director", "a certain bird"), and that thing is NOT the answer, you MUST name it outright in the question. A dangled unnamed noun BAITS chat into typing THAT noun instead of the real answer, even when the actual ask is perfectly clear. This is broken EVEN IF the answer-type is obvious. Failure: "This curved blade was a Southeast Asian farming tool, shaped to mimic the claws of what animal?" -> chat types "karambit" (the blade), answer was "tiger". FIX: "The karambit, a Southeast Asian farming tool, was shaped to mimic the claws of what animal?" Now the blade is given and chat aims at the animal. Same for "This crustacean's punch makes a flash called what?" (chat types "mantis shrimp", answer "sonoluminescence") -> "The mantis shrimp's punch...". The ONLY time an unnamed subject is allowed is a pure guess-the-subject riddle where that unnamed thing IS the answer. Otherwise: name it.
...
THE ANSWER STAYS IN THE TOPIC'S WORLD. Whoever asked for this topic wants to win with their knowledge OF it. A cross-domain fact is great material, but orient the question so the thing the player must NAME belongs to the topic's own world — put the outsider in the question, keep the in-world entity as the answer. (Topic "wrestling", fact "Samuel Beckett drove young Andre the Giant to school": ask "which wrestling legend did Nobel playwright Samuel Beckett drive to school?" → Andre the Giant. NEVER "name the playwright" → Beckett — a wrestling fan can't answer that with wrestling knowledge.)
...
export const LENSES = [
  ...
  'a hidden connection to something seemingly unrelated — put the outsider in the question and keep the in-world thing as the answer',
...
- UNNAMED-SUBJECT BAIT (reject aggressively — the #1 chat complaint) — the question refers to a specific thing WITHOUT NAMING IT ("this curved blade", "this crustacean", "this 2004 film", "a certain director") and that unnamed thing is NOT the answer. Chat reflexively types the unnamed thing instead of the real answer, so the round is baited — REJECT even if the question's actual ask is perfectly clear and self-contained. "This curved blade was a Southeast Asian farming tool, shaped to mimic the claws of what animal?" (answer "tiger") -> chat types "karambit" -> REJECT; it must name the blade: "The karambit... mimics the claws of what animal?". "This crustacean's punch makes a flash called what?" (answer "sonoluminescence") -> chat types "mantis shrimp" -> REJECT. The ONLY pass is a pure guess-the-subject riddle where the unnamed thing IS the answer. If a nameable subject is dangled unnamed and something else is the answer, reject.
...
- OFF-TOPIC — when a TOPIC line is given, the question MUST genuinely be about that topic: either about the topic itself, or about a person/place/work/event/thing that clearly BELONGS to it. A question on an unrelated subject that drifted in by mistake must be rejected (asked about "Romania" but the question is about a Diablo speedrun -> reject). Be generous about what belongs to a topic — a Romanian gymnast IS on-topic for "Romania", a boss in a game IS on-topic for that game, and a fact about a subject's creator, franchise, series, studio, or country of origin IS on-topic (a deliberate zoom-out, not drift). ALSO reject when the ANSWER itself lives in a different world than the topic: the question may bridge domains, but the entity the player must name should belong to the topic's own world, something a fan of the topic could know from following it (topic "wrestling", answer "Samuel Beckett" -> reject even though the question mentions a wrestler — the fact should be flipped so the wrestler is the answer). Reject only a REAL mismatch (unrelated subject, or an out-of-world answer), never a fair in-world tangent. If no TOPIC line is given, skip this check.
